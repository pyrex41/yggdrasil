...
| `scip-check PROG OUTDIR --target go` | stage-5 level-2 oracle: build the shaken program and the full program with the same builder, recover the KL-level call graph from each generated module with `go/ast` (bindings are nodes, `PrimFunc` lookups are edges), and assert by name that every defun the shake kept is reachable in the graph the backend emitted, and that everything the shaken build reaches the full build reaches too. The only subtraction is the target's declared `special_forms` in [`builders.json`](builders.json) — the names the port lowers with no symbol lookup, each cited to a line of the port — plus the user's own defuns and the synthesised `shen.initialise`; the declaration is itself audited, and any other residue is printed name by name and fails. Prints `OK footprint=N reachable=N accounted=N` or `FAIL` with `kl-delta-missing` / `kl-delta-extra` / `kl-missing-in-full` / `special-forms-unknown` lines, each `kl-delta-missing` annotated with whether the KL calls that name at all — see [`docs/analysis-rules.md`](docs/analysis-rules.md) |
