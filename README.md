...
| `lower PROG OUTDIR --target T` | shake as usual, then write `OUTDIR/lowered/`: a byte copy of the slice with every kernel defun in `T`'s declared `native_overrides` deleted from `kernel.kl`, plus `yggdrasil.lowering.txt` naming each drop and its [`builders.json`](builders.json) citation. `OUTDIR` itself is never modified — the canonical slice stays the artifact of record. Gated on `native_overrides_installed_after`: refuses (exit 1, no `lowered/`) unless the port installs its natives before `shen.initialise`, which no target does today — see [`docs/lowering.md`](docs/lowering.md) |
