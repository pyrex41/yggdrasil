...
yggdrasil scip-check prog.shen out/ --target go # stage-5 oracle: the shake's footprint vs the artifact's own graph, by name
...
| `scip-check PROG OUTDIR --target go` | stage-5 level-2 oracle: build the shaken program and the full program with the same builder, recover the KL-level call graph from each generated module with `go/ast` (the run-time defun bindings are the nodes, the symbol occurrences in their bodies — in any position, as the shake's own rules count them — are the edges), and assert by name that every defun the shake kept is reachable in the graph the backend emitted, and that everything the shaken build reaches the full build reaches too. The only subtraction is the target's declared `special_forms` in [`builders.json`](builders.json) — the names the port lowers with no symbol lookup, each cited to a line of the port — together with whatever only those reach, plus the user's own defuns and the synthesised `shen.initialise`; the declaration is itself audited, and any other residue is printed name by name and fails. Prints `OK footprint=N reachable=N accounted=N` or `FAIL` with `kl-delta-missing` / `kl-delta-extra` / `kl-missing-in-full` / `special-forms-unknown` lines — see [`docs/analysis-rules.md`](docs/analysis-rules.md) |
...
| `lower PROG OUTDIR --target T` | shake as usual, then write `OUTDIR/lowered/`: a byte copy of the slice with every kernel defun in `T`'s declared `native_overrides` deleted from `kernel.kl`, plus `yggdrasil.lowering.txt` naming each drop and its [`builders.json`](builders.json) citation. `OUTDIR` itself is never modified — the canonical slice stays the artifact of record. Gated: refuses (exit 1, no `lowered/`) unless the port declares `native_overrides_installed_after` as `none`/`before-initialise` and names a test in `native_overrides_checked_by`. `go` declares both as of shen-go `30ab469`, so the gate opens there and `lower tests/fib.shen out --target go` writes a slice with 18 defuns dropped — but that slice does **not** run: `lower-check` reports `FAIL pair=none run=lowered@go`, because shen-go installs a native only for a name the kernel already bound — see [`docs/lowering.md`](docs/lowering.md) |
| `lower … --report-only` | measurement only, never gated: prints `kept-kernel-defuns=N native-overridden=M declared=K` and the names, for the shaken slice against `T`'s declaration. 18 of 54 on `tests/fib.shen --target go` |
| `lower-check PROG OUTDIR --target T` | the three-way parity that is the evidence for a lowering: canonical slice on the reference host, canonical on `T`, lowered on `T`, each against `tests/PROG.expected` where present. Prints `yggdrasil-lower-check: OK …` or `FAIL pair=… ` naming which pair disagreed; when the gate refuses, `SKIP reason=… ` and exit 0, because that is a declared fact about the port and not a failure. On `go` at shen-go `30ab469` it FAILs on `fib`, `hello` and `prolog` — see [`docs/lowering.md`](docs/lowering.md), "The parity, actually run, and what it found" |
...
**Complete pinned toolchain (Nix).** [`flake.nix`](flake.nix) supplies the Go
...
The Shen implementations remain sibling source checkouts (or paths selected by
