...
| `lower-check PROG OUTDIR --target T` | the three-way parity that is the evidence for a lowering: canonical slice on the reference host, canonical on `T`, lowered on `T`, each against `tests/PROG.expected` where present. Prints `yggdrasil-lower-check: OK …` or `FAIL pair=… ` naming which pair disagreed; when the gate refuses, `SKIP reason=… ` and exit 0, because that is a declared fact about the port and not a failure — see [`docs/lowering.md`](docs/lowering.md) |
